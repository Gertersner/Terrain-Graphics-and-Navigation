...
void init()
{
   ...
   // terrain_height_field->seed = 12345;  // use this line to keep the terrain the same every run
    terrain_height_field->seed = clock();  // use this line to change the terrain every run
...
//    terrain_height_field->ping_pong_strength = 7;
    ...
    auto* artf_cube = new cs4722::artifact();
    artf_cube->the_shape = new cs4722::block();
    artf_cube->world_transform.scale = glm::vec3(2, 4, 2);
    artf_cube->world_transform.translate = glm::vec3(20, terrain_height_field->altitude(20, 20)+2, 20);

    auto* artf_cylinder = new cs4722::artifact();
    artf_cylinder->the_shape = new cs4722::cylinder();
    artf_cylinder->world_transform.scale = glm::vec3(2, 4, 2);
    artf_cylinder->world_transform.translate = glm::vec3(40, terrain_height_field->altitude(40,40)+2, 40);

    auto* artf_sphere = new cs4722::artifact();
    artf_sphere->the_shape = new cs4722::sphere();
    artf_sphere->world_transform.scale = glm::vec3(2, 4, 2);
    artf_sphere->world_transform.translate = glm::vec3(50, terrain_height_field->altitude(50,50)+4, 50);

    auto* artf_torus = new cs4722::artifact();
    artf_torus->the_shape = new cs4722::torus();
    artf_torus->world_transform.scale = glm::vec3(2, 4, 2);
    artf_torus->world_transform.translate = glm::vec3(80, terrain_height_field->altitude(80,80)+4, 80);

    artf_list.push_back(artf_cube);
    artf_list.push_back(artf_cylinder);
    artf_list.push_back(artf_sphere);
    artf_list.push_back(artf_torus);
...
void general_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    */
    ...
    if(movement_restricted) {
        ...
        auto x = the_view->camera_position.x;
        auto z = the_view->camera_position.z;
        auto y = terrain_height_field->altitude(x, z) + 1;
        the_view->camera_position = glm::vec4(x, y, z, 1.0);

        if (y > max_height || x < 0 || x > terrain_scale.x + terrain_translate.x || z < 0 || z > terrain_scale.z + terrain_translate.z) {
